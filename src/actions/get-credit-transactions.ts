...
export const getCreditTransactionsAction = userActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    try {
      ...
      // Search logic: text fields use case-insensitive LIKE, and if search is a number, also search amount fields
      // Use lower() for case-insensitive search in SQLite (Cloudflare D1)
      if (search) {
        const searchConditions = [];
        const searchPattern = `%${search.toLowerCase()}%`;
        // Always search text fields
        searchConditions.push(
          sql`lower(${creditTransaction.type}) LIKE ${searchPattern}`,
          sql`lower(${creditTransaction.paymentId}) LIKE ${searchPattern}`,
          sql`lower(${creditTransaction.description}) LIKE ${searchPattern}`
        );
