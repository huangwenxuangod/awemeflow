...
export async function canAddCreditsByType(userId: string, creditType: string) {
  ...
  // SQLite uses strftime() instead of EXTRACT() in PostgreSQL,
  // createdAt is stored as milliseconds timestamp, so we divide by 1000
  const existingTransaction = await db
    ...
    .where(
      and(
        ...
        sql`CAST(strftime('%m', datetime(${creditTransaction.createdAt} / 1000, 'unixepoch')) AS INTEGER) = ${currentMonth + 1}`,
        sql`CAST(strftime('%Y', datetime(${creditTransaction.createdAt} / 1000, 'unixepoch')) AS INTEGER) = ${currentYear}`
      )
    )
    .limit(1);
